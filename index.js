...
async function run() {
  ...
  try {
    ...
    status.mainCompile = await utils.checkExec('mvn', {
      param: ['-ntp', '"-DcompileOptionXlint=-Xlint:all"', '"-DcompileOptionXdoclint=-Xdoclint:all/private"', '-DcompileOptionFail=false', '-Dmaven.compiler.failOnWarning=false', '-Dmaven.compiler.showWarnings=true', 'compile'],
